<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="name" label="仓库名" width="240">
      </el-table-column>
...
export default {
  ...
  methods:{
    ...
    deleteData(id) {
      this.$axios.get(this.$httpUrl + '/storage/delete?id=' + id).then(result => result.data).then(result => {
        if (result.code == 200) {
          this.$message({
            message: '删除成功',
            type: 'success'
    ...
    doModify(){
      this.$axios.post(this.$httpUrl + '/storage/modify', this.form).then(result => {
    ...
    doSave() {
      this.$axios.post(this.$httpUrl + '/storage/save', this.form).then(result => {
        // 建议使用 result.data.code
        if (result.data.code == 200) {
          this.$message({
            message: '操作成功',
